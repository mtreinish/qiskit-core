...
cdef void increment_node(unsigned int node, cmap[unsigned int, unsigned int] cmap):
    if cmap.count(node):
        cmap[node] += 1
    else:
        cmap[node] = 1
...
def heuristic_search(cpplist[unsigned int] front_layer, object dag,
    ...
    """
    cdef cmap[unsigned int, unsigned int] applied_predecessors
    ...
    for _, input_node in dag.input_map.items():
        for successor in _successors(input_node._node_id, dag):
            increment_node(successor, applied_predecessors)
    ...
    while not front_layer.empty():
        ...
        if not execute_gate_list.empty():
            for node_id in execute_gate_list:
                ...
                for successor in _successors(node._node_id, dag):
                    increment_node(successor, applied_predecessors)
                    if is_resolved(applied_predecessors, successor, dag):
...
def _successors(unsigned int node, object dag):
    for _, successor, edge_data in dag._multi_graph.out_edges(node):
        if not isinstance(dag._multi_graph[successor], DAGOpNode):
            continue
        if isinstance(edge_data, Qubit):
            yield successor

cdef bool is_resolved(cmap[unsigned int, unsigned int] applied_gates, object node, object dag):
    """Return True if all of a node's predecessors in dag are applied."""
    return applied_gates[node] == len(dag._multi_graph[node].qargs)
