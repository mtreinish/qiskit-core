...
class MatplotlibDrawer:
    ...
    def _multiqubit_gate(self, xy, n_qubits, fc=None, wide=True, text=None,
                         subtext=None):
        xpos, ypos = xy
        if wide:
            wid = WID * 2.8
        ...
        box = patches.Rectangle(
           xy=(xpos - 0.5 * wid, ypos - 0.5 * n_qubits * height), width=wid,
            height=height, fc=_fc, ec=self._style.lc, linewidth=1.5,
            zorder=PORDER_GATE)
            ...
            for op in layer:
                ...
                #
                    ...
                    # Custom gate
                        ...
                        self._multiqubit_gate(q_xy[0], 2, text=op.name)
                #
                ...
                #
                    ...
                    # custom gate
                        ...
                        self._multiqubit_gate(q_xy[0], 3, text=op.name)

                # draw custom multi-qubit gate
                elif len(q_xy) > 3:
                    self._multiqubit_gate(q_xy[0], len(q_xy), text=op.name)
                else:
