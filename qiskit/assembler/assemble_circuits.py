...
def fast_assemble_circuits(circuits, run_config, qobj_id, qobj_header):
    """Assembles a list of circuits into a qobj which can be run on the backend.
    ...
    for circuit in circuits:
        ...
        for op_context in circuit.data:
            ...
            # to the conditional instruction to map the creg ?= val condition
            # onto a gating register bit.
            if getattr(instruction, '_condition'):
                ctrl_reg, ctrl_val = instruction._condition
                mask = 0
                val = 0
                for clbit in clbit_labels:
        ...
        experiments.append(FastQasmExperiment(config, header, instructions))
        if n_qubits > max_n_qubits:
            max_n_qubits = n_qubits
        if memory_slots > max_memory_slots:
            max_memory_slots = memory_slots
