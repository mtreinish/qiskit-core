...
from qiskit.qobj.fast_qobj import FastQasmQobj, FastQasmExperiment, FastRunConfig, FastQasmInstruction

def assemble_circuits(circuits, run_config, qobj_id, qobj_header, fast=False):
    ...
    """
    if fast:
        QasmQobjConfig = FastRunConfig
        QasmQobjExperiment = FastQasmExperiment
        QasmQobj = FastQasmQobj
        QasmQobjInstruction = FastQasmInstruction
    else:
        from qiskit.qobj import (QasmQobj, QasmQobjInstruction, QasmQobjExperiment,
                                 QasmQobjConfig)
    ...
    for circuit in circuits:
        ...
        for op_context in circuit.data:
            if fast:
                instruction = op_context[0].fast_assemble()
            else:
