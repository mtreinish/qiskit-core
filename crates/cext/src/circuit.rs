...
use qiskit_circuit::operations::{Operation, Param, StandardGate, StandardInstruction, UnitaryGate, ArrayType};
...
/// Append a new Unitary Gate to the circuit
///
/// A unitary gate is a gate represented solely by it's unitary matrix
///
/// # Safety
///
/// Behavior is undefined ``circuit`` is not a valid, non-null pointer to a ``QkCircuit``.
///
/// `array` must be non-null, and it must be safe to [`.offset()`] `ptr` by zero.
...
#[no_mangle]
#[cfg(feature = "cbinding")]
pub unsafe extern "C" fn qk_circuit_append_unitary_gate(
    circuit: *mut CircuitData,
    array: *const Complex64,
    num_qubits: u32,
    qubits: *const u32,
) -> ExitCode {
    let circuit = unsafe { mut_ptr_as_ref(circuit) };
    let shape = (2_usize.pow(num_qubits), 2_usize.pow(num_qubits));
    let matrix: ArrayView2<Complex64> = unsafe {
        ArrayView2::from_shape_ptr(shape, array)
    };
    let unitary = UnitaryGate {
        array: ArrayType::NDArrayView(matrix),
    };
    let qubits: Vec<Qubit> = unsafe {
        (0..num_qubits)
            .map(|idx| Qubit(*qubits.wrapping_add(idx as usize)))
    ...
    circuit.push_packed_operation(
        PackedOperation::from_unitary(Box::new(unitary)),
        &[],
        &qubits,
        &[],
    );
