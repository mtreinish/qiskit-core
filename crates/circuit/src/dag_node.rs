...
impl DAGInNode {
    #[new]
    fn py_new(wire: Py<PyAny>) -> PyResult<PyClassInitializer<Self>> {
        Ok(PyClassInitializer::from(DAGNode { node: None }).add_subclass(DAGInNode { wire }))
    }
...
impl DAGOutNode {
    #[new]
    fn py_new(wire: Py<PyAny>) -> PyResult<PyClassInitializer<Self>> {
        Ok(PyClassInitializer::from(DAGNode { node: None }).add_subclass(DAGOutNode { wire }))
    }
